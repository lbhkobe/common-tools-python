...
class JDYAutomationPlaywright:
    ...
    def navigate_to_sales_outbound(self):
        """导航到销售订单页面"""
        try:
    ...
    def click_history_orders(self):
        ...
        try:
            ...
            history_selectors = [
                "text=历史单据",
                "button:has-text('历史单据')",
                "span:has-text('历史单据')",
                "div:has-text('历史单据')",
                "a:has-text('历史单据')",
                "[class*='button']:has-text('历史单据')"
            ]
                ...
                try:
                    ...
                    if locator.count() > 0:
                        for i in range(locator.count()):
                            ...
                            if elem.is_visible():
                                history_button = elem
                                logger.info(f"找到历史单据按钮: {selector}")
                                break
                    if history_button:
                        break
                except:
            ...
            if not history_button:
                logger.error("未找到历史单据按钮")
                self.page.screenshot(path="debug_no_history_button.png")
                logger.info("已保存截图: debug_no_history_button.png")
                return False
            
            # 滚动到按钮可见
            history_button.scroll_into_view_if_needed()
            time.sleep(1)
            ...
            logger.info("已进入销售出库单列表页")
            return True
            
        except Exception as e:
            logger.error(f"点击历史单据出错: {str(e)}")
            self.page.screenshot(path="error_click_history.png")
            return False
        ...
        try:
                ...
                if tables.count() > 0:
                    for i in range(tables.count()):
                        ...
                        for j in range(header_cells.count()):
                            headers.append(header_cells.nth(j).text_content().strip())
                        
                        if headers:
                            logger.info(f"表头: {headers}")
                        
                        # 获取数据行
                        for k in range(1, rows.count()):
                            row = rows.nth(k)
                            ...
                            row_data = {}
                            for m in range(cells.count()):
                                key = headers[m] if m < len(headers) else f"column_{m}"
                                row_data[key] = cells.nth(m).text_content().strip()
                            
                            if row_data and any(row_data.values()):
                                data.append(row_data)
                        
                        if data:
                            logger.info(f"从表格获取到 {len(data)} 条数据")
                            return data
                            
            except Exception as e:
                logger.warning(f"方法1(表格)获取数据失败: {str(e)}")
            ...
            try:
                ...
                for selector in list_selectors:
                    list_items = self.page.locator(selector)
                    if list_items.count() > 1:
                        logger.info(f"使用选择器 {selector} 找到 {list_items.count()} 个项目")
                        ...
                        if data:
                            logger.info(f"从列表容器获取到 {len(data)} 条数据")
                            return data
                            
            except Exception as e:
                logger.warning(f"方法2(列表容器)获取数据失败: {str(e)}")
            
            # 方法3: 保存页面源码供分析
            logger.info("尝试保存页面源码以供分析...")
            ...
            self.page.screenshot(path="list_page_screenshot.png")
            logger.info("页面截图已保存到 list_page_screenshot.png")
            
            # 返回基本页面信息
            return [{
                "url": self.page.url,
                "title": self.page.title(),
                "message": "请检查 page_source.html 和 list_page_screenshot.png 以确定正确的数据选择器"
            }]
            
        except Exception as e:
            logger.error(f"获取列表数据出错: {str(e)}")
            return []
    
    def save_data(self, data, filename="sales_outbound_data.json"):
        """保存数据到JSON文件"""
        try:
    ...
    def run(self):
        ...
        try:
            ...
            # 导航到销售出库
            if not self.navigate_to_sales_outbound():
                return []
